    ...
    try:
        ...
        if not user_doc:
            return JSONResponse(status_code=404,
                                content={"message": f"A user with the username '{username_input}' was not found"})
        ...
        return JSONResponse(status_code=500, content={"message": "Something went wrong: " + str(e)})
    ...
    try:
        ...
        if existing_user_username:
            return JSONResponse(status_code=409,
                                content={"message": f"A user with username '{username_input}' already exists"})
        existing_user_email = user_service.find_by_email(signup_input.get("email"))
        if existing_user_email:
            return JSONResponse(status_code=409,
                                content={"message": f"A user with email '{email_input}' already exists"})
